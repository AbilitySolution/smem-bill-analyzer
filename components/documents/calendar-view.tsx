...
import type { CalendarDay, PeriodFilter } from "@/lib/data/invoice-list-params";
...
/**
 * Heatmap annuelle des dates de facture. `days` est agrégé en SQL sur TOUT le périmètre
 * filtré (la liste, elle, est paginée) — voir `getInvoiceCalendarDays`.
 */
export function CalendarView({ days, onPick }: { days: CalendarDay[]; onPick: (p: PeriodFilter) => void }) {
  const years = useMemo(() => {
  ...
  const [yearRaw, setYear] = useState<number | null>(null);
  // L'année par défaut suit les données : si les filtres changent et que l'année choisie
  // disparaît du périmètre, on retombe sur la plus récente disponible plutôt que d'afficher
  // une grille vide.
  const year = yearRaw != null && years.includes(yearRaw)
    ? yearRaw
  ...
  const byDay = useMemo(() => {
    ...
    for (const d of days) {
      ...
      map.set(d.date, { count: d.count, ttc: d.ttc });
    }
    return map;
  }, [days, year]);
    ...
    <div className="rounded-xl border border-[var(--kn-border)] bg-[var(--kn-card)] p-5">
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-1">
          <button
            onClick={() => canPrev && setYear(years[idx - 1])}
            disabled={!canPrev}
            aria-label="Année précédente"
            className="cursor-pointer rounded-md p-1.5 text-[var(--kn-text-muted)] enabled:hover:bg-[var(--kn-active)] disabled:cursor-default disabled:opacity-30"
          >
            <ChevronLeft className="size-4" />
          </button>
          <span className="min-w-[3.5rem] text-center text-[15px] font-bold tabular-nums text-[var(--kn-text)]">{year}</span>
          <button
            onClick={() => canNext && setYear(years[idx + 1])}
            disabled={!canNext}
  ...
  return (
    <div className="overflow-x-auto pb-1">
      <div className="inline-block">
            ...
            <div key={ci} className="flex flex-col gap-[3px]">
              {w.map((cell, ri) =>
                cell ? (
                  <button
                    key={cell.date}
                    onClick={() => cell.count > 0 && onPick({ from: cell.date, to: cell.date, label: frLong(cell.date) })}
                    title={`${frLong(cell.date)} · ${cell.count} ${plural(cell.count)}${cell.count ? ` · ${eurShort(cell.ttc)}` : ""}`}
                    className={cx(
                      "rounded-[3px] ring-offset-1 transition-transform focus:outline-none",
                      cell.count > 0
                        ? "cursor-pointer hover:scale-110 hover:ring-1 hover:ring-[#ea580c] focus:ring-1 focus:ring-[#ea580c]"
                        : "cursor-default",
                    )}
        ...
        {/* Légende */}
        <div className="ml-6 mt-3 flex items-center gap-1.5 text-[10px] text-[var(--kn-text-muted)]">
          <span>Moins</span>
  ...
  return (
    <div className={cx("grid gap-3", gran === "trimestre" ? "grid-cols-2 lg:grid-cols-4" : "grid-cols-1 sm:grid-cols-2")}>
      {stats.map((s) => (
        <button
          key={s.label}
          onClick={() => s.count > 0 && onPick({ from: s.from, to: s.to, label: `${s.label} ${year}` })}
          className={cx(
            "group flex flex-col gap-3 rounded-xl border border-[var(--kn-border)] bg-[var(--kn-panel)] p-4 text-left transition-colors",
            s.count > 0 ? "cursor-pointer hover:border-[#f97316] hover:bg-[var(--kn-yellow-soft)]" : "cursor-default opacity-60",
          )}
        >
